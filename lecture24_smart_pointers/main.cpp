...
std::unique_ptr<FormatProcessor> create_processor(FormatType type, const std::string& filename) {
  // check is extension is ok
  if (type == FormatType::ASCII) {
    return std::make_unique<ASCIIProcessor>(filename);
  }
  if (type == FormatType::BINARY) {
    return std::make_unique<BinaryProcessor>(filename);
  }  
  if (type == FormatType::JSON) {
    return std::make_unique<JSONProcessor>(filename);
  }    
...
private:
  std::vector<std::unique_ptr<FormatProcessor>> processors_;
};

// Идиома RAII = Resource Acquition is an Initialization
